...
// getLatestPostsFromBlockchain fetches the latest posts from the blockchain and returns the raw response body.
func getLatestPostsFromBlockchain() ([]byte, error) {
	// Get the chain data from the connected node
	getChainAddress := fmt.Sprintf("%s/chain", ConnectedNodeAddress)
	response, err := http.Get(getChainAddress)
	if err != nil {
		return nil, fmt.Errorf("error fetching chain data: %v", err)
	}
	defer response.Body.Close()
	...
	return nil, fmt.Errorf("error fetching chain data: %s", response.Status)
}
...
func submitTextareaHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Print("postData", string(postData)) // Note: Use `string()` to print the JSON data as a string.
	...
	fmt.Print("postData", string(postData))
	...
	// Lock the mutex to safely append the new post to the postss slice
	mu.Lock()
	defer mu.Unlock()

	// Create a new post and append it to the postss slice
	newPost := Posts{
		Author:    author,
		Content:   postContent,
		Timestamp: time.Now().Unix(), // Use the current timestamp for the new post
	}
	postss = append(postss, newPost)
...
func homepageHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Print("posts data", postss)
	fmt.Println("post s        context data ", context)
	// Parse the template
	...
	// If the form is submitted, create the new post
	if r.Method == http.MethodPost {
		submitTextareaHandler(w, r)
		return
	}
